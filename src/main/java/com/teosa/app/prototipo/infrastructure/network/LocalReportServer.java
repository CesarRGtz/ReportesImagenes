...
import com.teosa.app.prototipo.infrastructure.persistence.UserIdentity;
    ...
    private void handle(HttpExchange exchange) throws IOException {
        try {
            ...
            if (path.equals("/health") && method.equals("GET")) {
                ...
                storage.saveCatalogEntry(read(exchange,CatalogEntry.class));sendOk(exchange);
            } else if (parts.length==3 && parts[1].equals("catalog") && method.equals("PUT")) {
                storage.updateCatalogEntry(decode(parts[2]),read(exchange,CatalogEntry.class));sendOk(exchange);
            } else if (parts.length==3 && parts[1].equals("catalog") && method.equals("DELETE")) {
                storage.deleteCatalogEntry(decode(parts[2]));sendOk(exchange);
            } else if (path.equals("/reports") && method.equals("GET")) {
                var reports=storage.listReports(query(exchange.getRequestURI(), "q"));
                // Older clients only understand photographic reports.
                if(!"ALL".equals(query(exchange.getRequestURI(), "kind")))
                    reports=reports.stream().filter(r->r.getKind()==DocumentKind.REPORT).toList();
                send(exchange, 200, reports);
                ...
                var templates=storage.listTemplates();
                if(!"ALL".equals(query(exchange.getRequestURI(), "kind")))
                    templates=templates.stream().filter(t->t.getKind()==DocumentKind.REPORT).toList();
                send(exchange, 200, templates);
